...
export const PreviewCard: React.FC<PreviewCardProps> = ({ card }) => {
  ...
  const fallbackBackground = `url("https://picsum.photos/seed/${occasion || 'greeting'}/600/800")`;
  ...
  return (
    <div className="w-full h-full flex justify-center items-center p-4 bg-slate-100/50 rounded-xl border border-slate-200 shadow-inner">
      ...
      >
        ...
        {!backgroundImage && (
          <div className="absolute inset-0 bg-gradient-to-br from-brand-100 to-indigo-100 flex items-center justify-center">
             <span className="text-slate-400 text-sm font-medium italic">AI Image will appear here...</span>
          </div>
        )}

        {/* Overlay for text readability */}
        <div className={`absolute inset-0 bg-black/20 pointer-events-none transition-opacity duration-300 ${backgroundImage ? 'opacity-100' : 'opacity-0'}`} />
